...
test -f /etc/resolv.conf && echo "/etc/resolv.conf is a regular file" || echo "/etc/resolv.conf is not regular file"
...
test -w /etc/resolv.conf && echo "/etc/resolv.conf is writeable" || echo "/etc/resolv.conf is not writable"
test -x /etc/resolv.conf && echo "/etc/resolv.conf is executable" || echo "/etc/resolv.conf is not an executable"
...
[ -w /tmp ] && echo "/tmp is writeable" || echo "/tmp is not  writable"
...
if ! (($secondNumber % $firstNumber == 0 )) ; then
  echo FirstNumber is multiple of SecondNumber
else
  echo FirstNumber is not a multiple of SecondNumber
fi
...
echo "Give first string"
read a
echo "Give second string "
read b
